...
export default function ImportStudents() {
    ...
    const handleImport = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || '${process.env.NEXT_PUBLIC_API_URL || "https://shmool.onrender.com"}'}` + '/students/bulk', {
                method: 'POST',
    ...
    const fetchDuplicates = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || '${process.env.NEXT_PUBLIC_API_URL || "https://shmool.onrender.com"}'}` + '/students/families/potential-duplicates');
            if (res.ok) {
    ...
    const handleMerge = async (family1: string, family2: string, relationType: string) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || '${process.env.NEXT_PUBLIC_API_URL || "https://shmool.onrender.com"}'}` + '/students/families/merge', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    const handleManualLink = async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || '${process.env.NEXT_PUBLIC_API_URL || "https://shmool.onrender.com"}'}` + '/students/families/manual-link', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
