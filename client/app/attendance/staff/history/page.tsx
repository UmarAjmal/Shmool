...
export default function StaffAttendanceHistoryPage() {
    ...
    const loadHistory = async ()=>{
        ...
        setLoading(true);
        try {
            const url = `${process.env.NEXT_PUBLIC_API_URL || "https://shmool.onrender.com"}/attendance/staff/history?month=${month}&year=${year}${deptId?`&department_id=${deptId}`:''}`;
                                ...
                                <table className="table table-hover align-middle mb-0" style={{minWidth:700}}>
                                    <thead style={{position:'sticky',top:0,zIndex:10,background:'var(--primary-dark)'}}>
                                        <tr>
                                            ...
                                            {visibleDates.map(d=>{ const {day,dow}=fmtDate(d); return (
