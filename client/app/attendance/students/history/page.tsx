...
export default function StudentAttendanceHistoryPage() {
    ...
    useEffect(()=>{
        fetch(`${process.env.NEXT_PUBLIC_API_URL || '${process.env.NEXT_PUBLIC_API_URL || "https://shmool.onrender.com"}'}` + '/academic').then(r=>r.json()).then(setClasses).catch(()=>{});
    },[]);
                                    ...
                                    {Object.entries(S_ABBR).map(([s,a])=>(
                                        <span key={s} style={{fontSize:'0.75rem',fontWeight:600,display:'flex',alignItems:'center',gap:4}}>
                                            <span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',
                                                          width:20,height:20,borderRadius:'50%',background:S_BG[s],color:S_COLOR[s],fontSize:'0.62rem',fontWeight:700}}>{a}</span>
                                            {s}
                                        </span>
                                    ))}
                                </div>
